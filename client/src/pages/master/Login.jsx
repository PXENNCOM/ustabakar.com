...
export default function MasterLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await authService.masterLogin({ phone });
      const { token, master } = res.data.data;
      login(token, { ...master, role: 'master' });
    ...
    <div className="min-h-screen bg-white flex items-center justify-center p-6 antialiased">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-10">
          <h1 className="text-2xl font-bold text-gray-900 tracking-tight">Usta Girişi</h1>
        ...
        <div className="border border-gray-100 rounded-2xl p-2 sm:p-4 space-y-6">
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            {/* Bilgi Notu (Daha temiz ve soft arka planlı) */}
        ...
        <p className="text-center text-xs text-gray-400 mt-6 font-medium">
          Müşteri misiniz?{' '}
          <Link to="/giris" className="text-gray-600 font-semibold hover:text-gray-900 transition-colors ml-1">
            Müşteri Girişi →
          </Link>
        </p>
